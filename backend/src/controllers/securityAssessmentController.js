...
const securityAssessmentController = {
    // Get all security assessments
    getAllAssessments: async (req, res) => {
        try {
            const result = await db.query(`
                SELECT sa.*, h.house_number, h.village_no, h.sub_district
                FROM security_assessment sa
                LEFT JOIN households h ON sa.household_id = h.household_id
                ORDER BY sa.assessment_id DESC`
            );
            res.json(result.rows);
        } catch (err) {
            console.error('Error in getAllSecurityAssessments:', err);
    ...
    getAssessmentsByHousehold: async (req, res) => {
        try {
            ...
            console.error('Error in getSecurityAssessmentsByHousehold:', err);
            res.status(500).json({ error: 'Internal server error' });
        }
    },
    ...
    getAssessmentById: async (req, res) => {
        try {
            ...
            console.error('Error in getSecurityAssessmentById:', err);
            res.status(500).json({ error: 'Internal server error' });
        }
    },
    ...
    createAssessment: async (req, res) => {
        try {
            ...
            console.error('Error in createSecurityAssessment:', err);
            res.status(500).json({ error: 'Internal server error' });
        }
    },
    ...
    updateAssessment: async (req, res) => {
        try {
            ...
            const result = await db.query(
                `UPDATE security_assessment SET
                    household_id = $1,
                    assessment_date = $2,
                    q1_score = $3,
                    q1_note = $4,
            ...
            console.error('Error in updateSecurityAssessment:', err);
            res.status(500).json({ error: 'Internal server error' });
        }
    },
    ...
    deleteAssessment: async (req, res) => {
        try {
            const { id } = req.params;
            const result = await db.query(
                'DELETE FROM security_assessment WHERE assessment_id = $1 RETURNING *',
                [id]
            );
            ...
            res.json({ message: 'Security assessment deleted successfully' });
        } catch (err) {
            console.error('Error in deleteSecurityAssessment:', err);
            res.status(500).json({ error: 'Internal server error' });
        }
    }
};

module.exports = securityAssessmentController;